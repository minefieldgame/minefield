...
export const INVENTORY_METRIC_LABELS: Record<keyof InventoryMetrics, { label: string; description: string }> = {
  ...
  previouslyUsed: { label: "Used (metric scope)", description: "Permanently reserved exact candidate identities already published for this game, within the metric scope named on the health card." },
  unusedEligible: { label: "Unused eligible (metric scope)", description: "Playable candidates still available after permanent duplicate exclusion within the metric scope named on the health card; this drives health." },
